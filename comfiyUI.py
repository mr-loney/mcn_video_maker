...
class ImageGenerateFrame(wx.Frame):
    def __init__(self, parent, folder_path):
        super().__init__(parent, title="工作流（多任务）", size=(900, 750))
        self.parent = parent
    ...
    def do_submit_in_thread(self):
        ...
        try:
            ...
            for _ in range(repeat_count):
                ...
                api = server_func.AsyncTask("MCNComfyUIFlow", single_submission)
                taskuuid = api.call()
    ...
    def upload_all_to_ftp(self, submission):
        """
        1) 收集 submission["flows"] 的 flow_api => 只保留本地 workflows_dir + flow_api
        2) 收集 submission["flows"] 的 images => 并发上传
        3) 替换
        """
        local_to_ftp = {}
        ftp_workflow_dir = "ftp://183.6.90.205:2221/mnt/NAS/mcn/workflows"
        ftp_reslib_base = "ftp://183.6.90.205:2221/mnt/NAS/mcn/reslib"

        # A) flows => flow_api
        #   flow_api 形如 "wf.json"
        #   本地路径 => os.path.join(self.workflows_dir, flow_api)
        #   FTP路径 => ftp_workflow_dir/wf.json
        for flow in submission["flows"]:
            flow_api = flow.get("flow_api", "")
            if flow_api:
                local_json_path = os.path.join(self.workflows_dir, flow_api)
                if os.path.isfile(local_json_path):
                    ftp_path = f"{ftp_workflow_dir}/{flow_api}"
                    local_to_ftp[local_json_path] = ftp_path

        # B) flows => images
        #   ftp => ftp_reslib_base/{timestamp}_{random}/ ...
        ...
        for flow in submission["flows"]:
            ...
            for node_id, node_data in flow_inputs.items():
                local_img_path = node_data.get("image")
                if local_img_path and os.path.isfile(local_img_path):
                    base = os.path.basename(local_img_path)
                    ftp_path = f"{ftp_reslib_dir}/{base}"
                    local_to_ftp[local_img_path] = ftp_path
                local_vid_path = node_data.get("video")
        ...
        # -- 替换 submission 里的 flow_api / images => ftp 路径
        for flow in submission["flows"]:
            flow_api = flow.get("flow_api", "")
            if flow_api:
                # "workflows_dir + flow_api" => local key
                local_json_path = os.path.join(self.workflows_dir, flow_api)
                if local_json_path in local_to_ftp:
                    flow["flow_api"] = local_to_ftp[local_json_path]

            # images
            flow_inputs = flow.get("flow_inputs", {})
            for node_id, node_data in flow_inputs.items():
                local_img_path = node_data.get("image")
                if local_img_path in local_to_ftp:
                    node_data["image"] = local_to_ftp[local_img_path]
                local_vid_path = node_data.get("video")
...
class TaskPanel(wx.Panel):
    ...
    def on_select_image(self, event, node_id, preview_bitmap):
        ...
        """
        wildcard = "Image files (*.png;*.jpg;*.jpeg;*.bmp;*.mp4;*.MP4;*.mov;*.avi;*.mkv)|*.png;*.jpg;*.jpeg;*.bmp;*.mp4;*.MP4;*.mov;*.avi;*.mkv"
    ...
    def get_task_data(self):
        flow_api = self.workflow_choice.GetStringSelection()
        flow_inputs = {}

        if self.workflow_data:
            # 处理 text/seed/steps/cfg/denoise
            for node_id, fields_dict in self.text_controls.items():
                flow_inputs[node_id] = {}
                for field_name, ctrl in fields_dict.items():
            ...
            # 但是优先使用 image_path_ctrl 的文本值
            for node_id in self.preview_bitmaps.keys():
                # 只要 node_id 有 upload，就说明需要 "image"
                # 先看 image_path_ctrl[node_id]
                local_image = ""
                if node_id in self.image_path_ctrl:
                    local_image = self.image_path_ctrl[node_id].GetValue().strip()
                # 如果 image_path_ctrl 为空，可能 fallback self.upload_paths[node_id]
                if not local_image and (node_id in self.upload_paths):
                    local_image = self.upload_paths[node_id]
                # 如果是文件夹 => pick random
                if os.path.isdir(local_image):
                    local_image = self.pick_random_image(local_image)
                if node_id not in flow_inputs:
                    flow_inputs[node_id] = {}
